...
public class RiordinaCodice2controller {
    ...
    private void setupGestureTarget(final TextArea target) {
        ...
        target.setOnDragDropped((DragEvent event) -> {
            ...
            if (db.hasString() && target.getText().isEmpty()) {
                target.setText(db.getString()); // Imposta il testo della destinazione con il contenuto del dragboard
                event.setDropCompleted(true); // Indica che l'operazione di drop è completata
            } else {
                event.setDropCompleted(false); // Indica che l'operazione di drop non è riuscita
            }
            event.consume(); // Consuma l'evento per evitare che venga propagato
        });
    }
        ...
        try (
            ...
            if (isCorrectOrder) {
                if (utenti.contains(utenteCorrente)) {
                    writer.println(utenteCorrente + " " + "es2II"); // Salva lo stato dell'utente nel file di salvataggi
                    App.increaseProgressII(); // Incrementa il progresso dell'utente
                    pwBar.println(utenteCorrente + " " + "0.66"); // Salva il progresso nel file del progresso
                    pwBar.close();
                }
    ...
    private void SalvaEContinua(ActionEvent e) throws Exception {
        try (
            ...
            if (isCorrectOrder) {
                ...
                App.setRoot("riordina_codice3"); // Cambia la scena al livello successivo
            } else {
                App.setRoot("home"); // Cambia la scena alla home in caso di ordine errato
            }
        } catch (Exception ex) {
