...
public class TrovaErrore3Controller {
    ...
    Boolean check = false;  // Flag per determinare se una risposta è stata selezionata
    ...
    protected void invioRisposta(ActionEvent e) throws Exception {
        ...
        // Se la risposta giusta (R4) è selezionata, visualizza "RISPOSTA ESATTA"
        if (R4.isSelected())
            label.setText("RISPOSTA ESATTA"); 
        // Se una risposta errata (R1, R2, R3) è selezionata, mostra il tipo di errore
        else if (R1.isSelected() || R2.isSelected() || R3.isSelected()) {
            label.setText("RISPOSTA ERRATA"); 
        }
    }
    ...
    protected void SalvaEdEsci(ActionEvent e) throws Exception {
        ...
        // Se la risposta giusta (R4) è selezionata
        if (R4.isSelected()) {
    ...
    protected void SalvaEContinua(ActionEvent e) throws Exception {
        ...
        // Se la risposta giusta (R4) è selezionata e la risposta è stata inviata
        if (R4.isSelected() && checkInvio == true) {
            // Aggiunge gli utenti dal file "utenti.txt"
            while (scf.hasNextLine()) {
                String s = scf.nextLine();
                String[] S = s.split(" ");
            ...
            pwBar.println(o + " " + "0.99");
            pwBar.close();  // Chiude il file della ProgressBar
            App.setRoot("es1Superato");  // Passa alla schermata successiva
        }
