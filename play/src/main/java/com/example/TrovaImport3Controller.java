...
public class TrovaImport3Controller  {

    // Dichiarazione dei ToggleButton per le risposte
    @FXML
    private ToggleButton R1;  // Risposta 1
    @FXML
    private ToggleButton R2;  // Risposta 2
    @FXML
    private ToggleButton R3;  // Risposta giusta

    // Pulsante per azioni successive
    @FXML
    ...
    protected void SalvaEContinua(ActionEvent e) throws Exception {
        ...
        if (R3.isSelected() && checkInvio == true) {
            ...
            pw.println(ri.get(0) + " " + "es3I");
            pwBar.println(o + " " + "0.99");  // Aggiorna la barra di progresso
    
            pwBar.close();  // Chiude il file della ProgressBar
            pw.close();  // Chiude il file di salvataggio
            App.setRoot("es2Superato");  // Passa alla schermata successiva
            App.conta++;  // Incrementa il contatore per il prossimo livello
        }

        // Log del contatore per il debug
