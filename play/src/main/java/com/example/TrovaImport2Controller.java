...
public class TrovaImport2Controller  {

    // Dichiarazione dei ToggleButton per le risposte
    @FXML
    private ToggleButton R1;  // Risposta giusta
    @FXML
    ...
    protected void SalvaEdEsci(ActionEvent e) throws Exception {
        ...
        // Se la risposta giusta (R1) è selezionata
        if (R1.isSelected()) {
    ...
    protected void SalvaEContinua(ActionEvent e) throws Exception {
        ...
        // Se la risposta giusta (R1) è selezionata e la risposta è stata inviata
        if (R1.isSelected() && checkInvio == true) {
            // Aggiunge gli utenti dal file "utenti.txt"
            while (scf.hasNextLine()) {
                String s = scf.nextLine();
                String[] S = s.split(" ");
            ...
            pwBar.close();  // Chiude il file della ProgressBar
            pw.close();  // Chiude il file di salvataggio
            App.setRoot("trovaimport3");  // Passa al prossimo livello
